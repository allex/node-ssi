...
module.exports = DirectiveHandler;
...
function DirectiveHandler(ioUtils) {
...
DirectiveHandler.prototype = {
    ...
    _handleInclude: function(attributes, currentFile, variables) {
        if (attributes.length !== 1) {
            return {error: 'Directive #include did not contain the correct number of attributes. "' + currentFile + '"'};
        } else if (attributes[0].name !== "virtual" && attributes[0].name !== "file") {
            return {error: 'Directive #include did not contain a "file" or "virtual" attribute. "' + currentFile + '"'};
        }
        ...
        try {
            ...
            if (output.trim().length === 0) {
                console.warn('WARING: Include directive contents empty. "' + currentFile + '#' + filename + '"')
            }
        } catch (e) {
            results.error = 'Parse directive failed. "' + currentFile + '" =>\n' + e;
            return results;
        }

        // Parse the contents of the file to handle SSI directives
        var parsed = this.parser.parse(this.ioUtils.resolvePath(currentFile, filename), results.output, variables);
